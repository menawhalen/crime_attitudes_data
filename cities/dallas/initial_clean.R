...
dat <- read_csv("cities/dallas/old_data/Police_Incidents_20250527.csv") %>% 
  clean_names()
######################################################
############# cleaning data
clean_dat <- dat %>%
  select(date1_of_occurrence, nibrs_crime_category, zip_code) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = ymd(date1_of_occurrence), # change to date
         ofns_desc = str_to_lower(nibrs_crime_category)) %>%  # make to lower case
  select(-c(date1_of_occurrence, nibrs_crime_category)) %>%
...
write_csv(final, "cities/dallas/current.csv")
write_csv(dat, "cities/dallas/full_uncleaned_current.csv")
