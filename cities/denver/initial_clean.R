...
dat <- read_csv("cities/denver/old_data/ODC_CRIME_OFFENSES_P_-3254178225590307312.csv") %>% 
  clean_names()
######################################################
############# cleaning data
clean_dat <- dat %>%
  select(first_occurrence_date, offense_type_id, geo_lat, geo_lon) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = mdy_hms(first_occurrence_date), # change to date
         ofns_desc = str_replace_all(offense_type_id, "-", " ")) %>%  # make to lower case
  select(-c(first_occurrence_date, offense_type_id)) %>%
  filter(date > ymd_hms("2015-12-31 23:59:59")) %>% 
  filter(!is.na(geo_lat) & !is.na(geo_lon)) %>% ## cant use if missing
  mutate(desc_crime_type = case_when(
    str_detect(ofns_desc, "murder|manslaughter|homicide") ~ "homicide",
    str_detect(ofns_desc, "of motor vehicle") ~ "mvt",
...
joined_zip <- st_join(st_as_sf(clean_dat, coords = c("geo_lon", "geo_lat"), crs = st_crs(zips)),
...
write_csv(final, "cities/denver/current.csv")
write_csv(dat, "cities/denver/full_uncleaned_current.csv")
