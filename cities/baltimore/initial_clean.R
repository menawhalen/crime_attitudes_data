...
dat <- read_csv("cities/baltimore/old_data/NIBRS_GroupA_Crime_Data_8914151926081121180.csv") %>% 
  clean_names()
...
############# cleaning data
clean_dat <- dat %>%
  select(crime_date_time, description, latitude, longitude) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = mdy_hms(crime_date_time), # change to date
         ofns_desc = str_to_lower(description)) %>%  # make to lower case
  select(-c(crime_date_time, description)) %>%
...
write_csv(final, "cities/baltimore/current.csv")
write_csv(dat, "cities/baltimore/full_uncleaned_current.csv")
