...
dat <- read_csv("cities/chicago/old_data/Crimes_-_2001_to_Present_20250527.csv") %>% 
  clean_names()
######################################################
############# cleaning data
clean_dat <- dat %>%
  select(date, primary_type, latitude, longitude) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = mdy_hms(date), # change to date
         ofns_desc = str_to_lower(primary_type)) %>%  # make to lower case
...
write_csv(final, "cities/chicago/current.csv")
write_csv(dat, "cities/chicago/full_uncleaned_current.csv")
