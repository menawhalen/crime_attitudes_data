...
dat <- read_csv("cities/detriot/old_data/RMS_Crime_Incidents.csv") %>% 
  clean_names()
######################################################
############# cleaning data
clean_dat <- dat %>%
  select(incident_occurred_at, offense_description, x, y) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = ymd_hms(incident_occurred_at), # change to date
         ofns_desc = str_to_lower(offense_description)) %>%  # make to lower case
  select(-c(incident_occurred_at, offense_description)) %>%
...
## 2016 not being fully reported so removed

final <- final %>% 
  filter(year != 2016)


write_csv(final, "cities/detriot/current.csv")
write_csv(dat, "cities/detriot/full_uncleaned_current.csv")
