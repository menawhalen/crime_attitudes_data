...
file_names <- list.files(path = "cities/DC/old_data", pattern = ".csv")

dat_files <- map(file_names, ~ read_csv(str_c("cities/DC/old_data/", .x), col_types = cols(CCN = col_character())) 
                 %>% clean_names)
...
############# cleaning data
clean_dat <- dat %>%
  select(start_date, offense, latitude, longitude) %>% ## usinged cmplt_fr_dt as date since is the earliest crime date and the most accurate time of the incident
  mutate(date = ymd_hms(start_date), # change to date
         ofns_desc = str_to_lower(offense)) %>%  # make to lower case
  select(-c(start_date, offense)) %>%
...
write_csv(final, "cities/DC/current.csv")
write_csv(dat, "cities/DC/full_uncleaned_current.csv")
